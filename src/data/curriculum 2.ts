...
export const SEASON_OVERVIEWS: SeasonOverview[] = [
  ...
  },
  {
    ...
    previewEpisodes: ['KOTRA 데이터베이스 탐색', '바이어 크레딧 검증', '첫 바이어 리스트 완성', '콜드 이메일 작성', '이메일 A/B 테스트', '팔로업 전략', 'Klaus 첫 접촉'],
  },
  {
    season: 3,
    title: '첫 미팅과 전시회',
    ...
    previewEpisodes: ['첫 화상 미팅 준비', '미팅 영어 핵심 표현', '전시회 네트워킹', '프레젠테이션 스킬', '바이어 니즈 파악', '미팅 후 팔로업', 'Klaus 첫 미팅'],
  },
  {
    season: 4,
    title: '가격 협상과 오퍼',
    ...
    previewEpisodes: ['Incoterms 완전 정복', '결제 조건 협상', '견적서 작성', '가격 방어 전략', '협상 유연성', '계약 전 체크리스트', 'Klaus 오퍼 협상'],
  },
  {
    season: 5,
    title: '계약과 선적',
    ...
    previewEpisodes: ['계약서 영어 이해', '선적 서류 준비', '납기 관리 커뮤니케이션', '클레임 대응', '사과와 해결책', '배송 문제 수습', 'Klaus 계약 완료'],
  },
  {
    season: 6,
    title: '관계 심화',
    ...
    previewEpisodes: ['인보이스 관리', '재오더 유도 이메일', '파트너십 제안', '연간 계약 협상', '관계 유지 전략', '추천인 마케팅', 'Klaus 장기 계약'],
  },
]
...
export const CURRICULUM: CurriculumEpisode[] = [
  ...
  // ── EP04 — Office Politics 101 ────────────────────────────────────────────
  {
    ...
    synopsis: '조사 중 GermanParts 발견, 구매 담당자로 Klaus Müller 확인. Sarah에게 보고하자 묘한 표정 변화. James도 그 이름을 어디서 들어봤다고 함. LinkedIn에서 Sarah와 Klaus가 공통 지인 1명.',
    phases: PHASE_DEFS,
  ...
  // ── EP05 — Email Mastery ──────────────────────────────────────────────────
  {
    ...
    synopsis: 'GermanParts로 보낼 첫 콜드 이메일 작성. James가 절반을 다시 씀. Sarah가 첨삭투성이로 돌려보냄. 세 번 다시 씀. 발송 직후 브로셔 첨부 누락 발견.',
    phases: PHASE_DEFS,
  ...
  // ── EP07 — Ready for Action (Season 1 Finale) ────────────────────────────
  {
    ...
    synopsis: '18일 침묵 끝에 Klaus로부터 짧고 건조한 회신. Sarah가 이름을 보고 조용해짐. James는 들떠있지만 Klaus가 까다롭다고 경고. Sarah가 사석에서 Klaus와 전 직장 동료였다고 처음 언급.',
    phases: PHASE_DEFS,
    unlockRequiresEpisode: 'ep06',
    expressions: [
      { id: 'ep07-e1', english: 'Thank you for getting back to me, Mr. Müller.', korean: '뮬러 씨, 회신 주셔서 감사합니다.', context: '회신 감사', learned: false, xp: 20 },
